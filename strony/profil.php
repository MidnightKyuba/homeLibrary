<?php
    session_start();
    if(!isset($_SESSION['userID']))
    {
        header('Location: ../index.php');
		exit();
    }
?>
<!DOCTYPE html>
<html lang="pl">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
	<meta name="author" content="Jakub Kittel">
	<meta name="description" content="Portal służy do zarządzania swoją biblioteką domową.">
	<meta name="keywords" content="bibloteka, domowa biblioteka, system zarządzania, zarządzanie, system zarządzania domową biblioteką">
    <title>Menu</title>
	<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-1BmE4kWBq78iYhFldvKuhfTAU6auU8tT94WrHftjDbrCEXSU1oBoqyl2QvZ6jIW3" crossorigin="anonymous">
	<link href="../css/style.css" rel="stylesheet">
	<script type="text/javascript" src="../skrypty/functions.js"></script>
	<script src="http://code.jquery.com/jquery-1.8.3.js" defer="defer"></script>
	<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/js/bootstrap.bundle.min.js" integrity="sha384-ka7Sk0Gln4gmtz2MlQnikT1wXgYsOg+OMhuP+IlRH9sENBO0LRn5q+8nbTov4+1p" crossorigin="anonymous"></script>
</head>
<body class="container-fluid">
	<div class="row naglowek">
		<h1 class="title col-xl-10 col-lg-8 col-md-6 col-sm-12 col-12 order-2">System zarządzania domową biblioteką</h1>
		<?php
			echo '<h4 class="col-xl-1 col-lg-2 col-md-3 col-sm-12 col-12 order-3">Witaj,'.$_SESSION['login'].'</h4>';
		?>
		<div class="col-xl-1 col-lg-2 col-md-3 col-sm-12 col-12 order-1">
			<div class="btn-group">
				<button type="button" class="btn btn-success dropdown-toggle" data-bs-toggle="dropdown">Menu</button>
				<div class="dropdown-menu">
					<a class="dropdown-item" href="booksbase.php">Baza książek</a>
					<a class="dropdown-item" href="mybooks.php">Moje książki</a>
					<a class="dropdown-item" href="profile.php">Profil</a>
					<?php
					if($_SESSION['rank']<3)
					{
						echo '<a class="dropdown-item" href="contact.php">Kontakt</a>';
					}
					else
					{
						echo '<a class="dropdown-item" href="#">Wiadomości</a>';
					}
					?>
					<a class="dropdown-item" href="../skrypty/logout.php">Wyloguj</a>
				</div>
			</div>
		</div>
	</div>
	<div class="row main">
		<div class="offset-xl-3 col-xl-6 offset-xl-3 offset-lg-3 col-lg-6 offset-lg-3 offset-md-3 col-md-6 offset-md-3 col-sm-12 col-12 mainlight">
		<?php
			echo "<p class='text'>Login: ".$_SESSION['login']."</p>";
			$connect = pg_connect();
			if($connect)
			{
				echo "<p class='text'>Email: ".pg_query($connect,"SELECT email FROM users WHERE user_id=".$_SESSION['userID']."")."</p>";
			}
			else
			{
				echo "<p class='text'>Nie można połączyć z bazą</p>";
			}
		?>
		<form action="../skrypty/changeEmail.php" method="post" autocomplete="on">
			<label class="form-label text">Zmień email</label>
			<input class="form-control" type="email" name="Email" required>
			<button class="btn btn-primary text" type="submit">Zmień</button>
		</form>
		<form action="../skrypty/changePassword.php" method="post" autocomplete="on">
			<legend class="text">Zmień hasło</legend>
			<label class="form-label text">Stare hasło</label>
			<input class="form-control" type="email" name="OldPassword" required>
			<label class="form-label text">Nowe hasło</label>
			<input class="form-control" type="email" name="NewPassword" required>
			<label class="form-label text">Powtórz nowe hasło</label>
			<input class="form-control" type="email" name="ControlNewPassword" required>
			<button class="btn btn-primary text" type="submit">Zmień</button>
		</form>
		<?php if($_SESSION[rank_id] == 1) :?>
		<form action="../skrypty/changeRank.php" method="post" autocomplete="on">
			<legend class="text">Zmień rangę użytkownika</legend>
			<label class="form-label text">Użytkownik:</label>
			<select name="UserId">
				<?php
					$connect = pg_connect();
					if($connect)
					{
						$results = pg_query($connect,"SELECT user_id, login FROM users ");
						foreach ($results as $result)
						{
							echo '<option value="'.$result['user_id'].'">'.$result['user_id'].'.'.$result['login'].'</option>';
						}
					}
					else
					{
						echo "<p class='text'>Nie można połączyć z bazą</p>";
					}
				?>
			</select>
			<label class="form-label text">Ranga:</label>
			<select name="RankId">
				<?php
					$connect = pg_connect();
					if($connect)
					{
						$results = pg_query($connect,"SELECT rank_id, rank_name FROM ranks ");
						foreach ($results as $result)
						{
							echo '<option value="'.$result['rank_id'].'">'.$result['rank_name'].'</option>';
						}
					}
					else
					{
						echo "Nie można połączyć z bazą";
					}
				?>
			</select>
			<button class="btn btn-primary text" type="submit">Zmień</button>
		</form>
		<?php endif; ?>
		</div>
	</div>
	<div class="row stopka bg-secondary">
		<div class="col-xl-4 col-lg-4 col-md-6 col-sm-12 col-12 order-1">
			<label class="form-label text">Zmień rozmiar czionki:</label>
			<input class="form-range" type="range" onchange="changeSizeText(this.value)" min="0" max="6" default="3" step="1">
		</div>
		<div class="col-xl-4 col-lg-4 col-md-0 col-sm-0 col-0 order-2">
		</div>
		<div class="col-xl-4 col-lg-4 col-md-6 col-sm-12 col-12 order-3">
			<p class="text">Copyright © 2022 . All rights reserved. Jakub Kittel</p>
		</div>
	</div>
</body>
</html>